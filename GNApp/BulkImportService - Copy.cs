    ...
    public class BulkImportService
    {
        ...
        public async Task<bool> DoImport(GNBulkImportStatus bulkImportStatus)
        {
            ...
            try
            {
                ...
                /**
                 * If there are groups to import, doImport
                 */
                if (totalRecordCountInGroupStaging > 0)
                {
                    int BATCH_START_IDX = 0;
                    int BATCH_MAX_SIZE = 1000;
                    int BATCH_ROW_IDX = 0;
                    ...
                    //load data set into db
                    Dictionary<string, object> lastGroupFileRecord = new Dictionary<string, object> 
                    {
                        {"ANALYSIS",new GNAnalysisRequest()},
                        {"CONTROL_GROUP",new GNAnalysisRequestGroup()},
                        {"COMPARISON_GROUP",new GNAnalysisRequestGroup()} 
                    };

                    foreach (GNBulkImportGroupStaging dataRow in db.GNBulkImportGroupStagings
                            .Where(s => s.GNBulkImportStatusId == bulkImportStatus.Id)
                            .OrderBy(s => s.ROW_IDX)
                            .Skip(BATCH_START_IDX)
                            .Take(BATCH_MAX_SIZE).ToList())
                    {
                        System.Console.WriteLine("----------------------\nImporting Group Record [" + BATCH_ROW_IDX + "]");
                        long insertImportRecordDataStartTime = DateTime.Now.ToFileTime();

                        lastGroupFileRecord = await InsertImportGroupRecordData(
                            dataRow, BATCH_ROW_IDX, bulkImportStatus.CreatedByContact, bulkImportStatus.Id, lastGroupFileRecord);
        ...
        private async Task<Dictionary<string, object>> InsertImportGroupRecordData(GNBulkImportGroupStaging dataRow, int dataRowIdx,
            GNContact userContact, Guid bulkImportStatusId, Dictionary<string, object> lastGroupFileRecord)
        {
            try
            {
                GNAnalysisRequestGroup group = await InsertGroup(dataRow, dataRowIdx, userContact, bulkImportStatusId, lastGroupFileRecord);

                InsertSamplesToGroup(group, bulkImportStatusId);

                InsertComparison(dataRow, dataRowIdx, group, userContact, bulkImportStatusId, lastGroupFileRecord);
                

            }
            catch (Exception e)
            {
                LogUtil.Error(logger, "Error inserting group comparison set at row idx = " + dataRowIdx, e);
                this.bulkImportLogService.LogError(userContact, bulkImportStatusId, "Error inserting sample file:"+e.Message, dataRowIdx + "");
            }

            return lastGroupFileRecord;
        }
        ...
        private async Task<GNAnalysisRequestGroup> InsertGroup(GNBulkImportGroupStaging dataRow, int dataRowIdx, GNContact userContact, Guid bulkImportStatusId, Dictionary<string, object> lastGroupFileRecord)
        {
            GNAnalysisRequestGroup group = null;

            string ms = "";
            string m2 = "";
            string groupName = dataRow.CONTROL_GROUP_NAME.ToString();
            string analysisName = dataRow.ANALYSIS_NAME.ToString();

            try
            {
                System.Console.WriteLine("------ searching analysisName " + analysisName);

                GNAnalysisRequest analysisRequest = this.db.GNAnalysisRequests
                                                                       .Where(t => (
                                                                           t.Description.ToLower() == analysisName.ToLower()
                                                                           && t.Project.Teams.FirstOrDefault().OrganizationId == userContact.GNOrganizationId
                                                                           )
                                                                       )
                                                                       .FirstOrDefault();
                ...
                {
                    GNAnalysisRequestGroup lastGroup = (GNAnalysisRequestGroup)lastGroupFileRecord["CONTROL_GROUP"];

                    if (groupName != null && lastGroup.Name == groupName)
                    {
                        group = lastGroup;
                    }
                    else
                    {
                        try
                        {
                            group = this.db.GNAnalysisRequestGroups
                            .Where(t => (
                                t.Name.ToLower() == groupName.ToLower()
                                && t.GNAnalysisRequest.Project.Teams.FirstOrDefault().OrganizationId == userContact.GNOrganizationId
                                )
                            )
                            .FirstOrDefault();
                        ...
                        catch (Exception e)
                        {
                            ...
                            System.Console.WriteLine("------No group found, OK, will create " + ms);
                        }
                        ...
                        if (group == null)
                        {
                            try
                            {
                                ...
                                System.Console.WriteLine("------group set up " + group.Id);
                                this.analysisRequestService.CreateAnalysisRequestGroup(analysisRequest.Id, group);

                                this.bulkImportLogService.LogSuccess(userContact, bulkImportStatusId, "Group '" + groupName + "' created.", dataRowIdx + "");
                            }
                            catch (Exception e2)
                            {
                                m2 = e2.Message;
                                System.Console.WriteLine("------cant create group " + m2);
                            }
                ...
                if (group == null)
                {
                    this.bulkImportLogService.LogError(userContact, bulkImportStatusId, "Group '" + groupName + "' not created due to unknown error.", dataRowIdx + "" + ms + m2);
                }
            }
            catch (Exception e)
            {
                System.Console.WriteLine("------ analysis Request not found " + e.Message);
            }
        ...
        private bool InsertSamplesToGroup(GNAnalysisRequestGroup group, Guid bulkImportStatusId)
        {
            System.Console.WriteLine("------>>>>>> InsertSamplesToGroup " + group.Name + " " + group.Id);
            try
            {
                IQueryable<GNBulkImportStaging> listOfSamples = db.GNBulkImportStaging.Where(a => a.GNBulkImportStatusId.Equals(bulkImportStatusId) && a.ANALYSIS_GROUP_NAME.Equals(group.Name));

                foreach (GNBulkImportStaging sample in listOfSamples)
                {
                    GNSample gnSample = db.GNSamples.Where(a => a.Name.Equals(sample.SAMPLE_NAME) && a.Organization.Id.Equals(group.GNAnalysisRequest.Project.Teams.FirstOrDefault().OrganizationId)).FirstOrDefault();

                    System.Console.WriteLine("------>>>> Group "+ group.Name + " and Sample "+ gnSample.Name);
                    ...
                    db.Database.ExecuteSqlCommand(
                        ...
                        new SqlParameter("@GNSamples_Id", gnSample.Id));
            ...
            {                
                System.Console.WriteLine("------>>>> Cannot insert Sample into Group " + e.Message);
            }
        ...
        private bool InsertComparison(GNBulkImportGroupStaging dataRow, int dataRowIdx, GNAnalysisRequestGroup group, GNContact userContact, Guid bulkImportStatusId, Dictionary<string, object> lastGroupFileRecord)
        {
            System.Console.WriteLine("------>>>> InsertComparison ");

            try
            {
                string analysisName = dataRow.ANALYSIS_NAME.ToString();
                string controlGroupName = dataRow.CONTROL_GROUP_NAME.ToString();
                string comparisonGroupName = dataRow.COMPARISON_GROUP_NAME.ToString();
                ...
                System.Console.WriteLine("------>>>> analysisRequest " + analysisRequest.Id + " -> " + analysisRequest.Description);

                GNAnalysisRequestGroup controlGroup = this.db.GNAnalysisRequestGroups
                                                                        .Where(t => (
                                                                            t.Name.ToLower() == controlGroupName.ToLower()
                                                                            && t.GNAnalysisRequest.Project.Teams.FirstOrDefault().OrganizationId == userContact.GNOrganizationId
                                                                            )
                                                                        )
                                                                        .FirstOrDefault();

                System.Console.WriteLine("------>>>> controlGroup " + controlGroup.Id + " -> " + controlGroup.Name);

                GNAnalysisRequestGroup comparisonGroup = this.db.GNAnalysisRequestGroups
                                                                        .Where(t => (
                                                                            t.Name.ToLower() == comparisonGroupName.ToLower()
                                                                            && t.GNAnalysisRequest.Project.Teams.FirstOrDefault().OrganizationId == userContact.GNOrganizationId
                                                                            )
                                                                        )
                                                                        .FirstOrDefault();

                System.Console.WriteLine("------>>>> comparisonGroup " + comparisonGroup.Id + " -> " + comparisonGroup.Name);

                GNAnalysisRequestGroupComparison groupComparison = db.GNAnalysisRequestGroupComparisons.Where(a => a.GNAnalysisRequestId.Equals(analysisRequest.Id) && a.GNAnalysisRequestControlGroupId.Equals(controlGroup.Id) && a.GNAnalysisRequestComparisonGroupId.Equals(comparisonGroup.Id)).FirstOrDefault();
                if (analysisRequest != null)
                {
                    this.analysisRequestService.CreateAnalysisRequestGroupAssociation(analysisRequest.Id, controlGroup.Id, comparisonGroup.Id);
                }
                
            }
            catch (Exception e)
            {
                LogUtil.Error(logger, "Error inserting group comparison set " + e.Message);
            }
