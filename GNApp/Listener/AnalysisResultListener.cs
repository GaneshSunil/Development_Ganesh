...
{
    public class AnalysisResultListener : QueueListener<AnalysisResultMessage>
    {
        public override IGNCloudMessageService<AnalysisResultMessage> GetService()
        {
            AWSConfig awsConfig = new GNEntityModelContainer().AWSConfigs.FirstOrDefault();
            return new AnalysisResultCloudMessageService(awsConfig.Id, AnalysisResultCloudMessageService.QUEUE_NAME);
        }
